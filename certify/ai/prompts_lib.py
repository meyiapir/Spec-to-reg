eng_prompt = """
            You are a system for checking specifications against regulations. Respond as accurately as possible, without unnecessary indentations. Analyze the following specifications for compliance with regulations: {specifications_data}
            DO NOT USE MARKDOWN OR ANOTHER FORMATING
            Description:
            The specifications must comply with technical standards. If the specifications contain incorrect, unclear, or improperly formulated terms (e.g., colloquial expressions or phrases that do not meet technical standards), highlight them on both sides with #- symbols (e.g., #-Машинка-#, #-Типо тесла-#) and indicate that they need to be replaced with accurate technical terms.  
            For each requirement, compare it with the regulation and indicate whether the regulations are followed. Add a comment if there are discrepancies between the specifications and the regulation, and highlight problematic terms in the comments.
            
            Response:
            Return the results in JSON format for each requirement, where:
            - `"validation"` — information about compliance with the specifications, highlighting incorrect terms.
...
rus_prompt = """
            Ты система для сверки спецификации с регламентами. Отвечай максимально точно, без лишних отступлений. Проанализируй следующие требования на соответствие регламентам: {specifications_data}
            НЕ ИСПОЛЬЗУЙ MARKDOWN ИЛИ ДРУГОЕ ФОРМАТИРОВАНИЕ
            Описание:
